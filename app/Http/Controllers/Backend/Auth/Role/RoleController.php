...
//        return 'fuck stick';
	...
	public function store(StoreRoleRequest $request)
	{
		...
		return redirect()->route('dashboard.auth.role.index')->withFlashSuccess(__('alerts.backend.roles.created'));
	}
	...
	{
		if ($role->isAdmin()) {
			return redirect()->route('dashboard.auth.role.index')->withFlashDanger('You can not edit the administrator role.');
		}
	...
	public function update(UpdateRoleRequest $request, Role $role)
	{
		...
		return redirect()->route('dashboard.auth.role.index')->withFlashSuccess(__('alerts.backend.roles.updated'));
	}
	...
	{
		if ($role->isAdmin()) {
			return redirect()->route('dashboard.auth.role.index')->withFlashDanger(__('exceptions.backend.access.roles.cant_delete_admin'));
		}
		...
		return redirect()->route('dashboard.auth.role.index')->withFlashSuccess(__('alerts.backend.roles.deleted'));
	}
}
